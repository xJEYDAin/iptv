...
class Generator:
    ...
    def generate(self, grouped: Dict[str, List], output_path: str = None) -> str:
        """生成单个 m3u 文件"""
        lines = [
        ...
        # 按分类排序
        sorted_items = sorted(grouped.items(), key=lambda x: sort_key(x[0]))

        for group_name, channels in sorted_items:
            if self.sort:
                channels = sorted(channels, key=lambda c: c.name)

            lines.append(f"#EXTGRP:{group_name} ({len(channels)})")
            for ch in channels:
                lines.append(build_extinf(ch, group_name if self.include_group_tag else ""))
                lines.append(ch.url)
    ...
    def generate_multi(
        self, grouped: Dict[str, List], output_dir: str, prefix: str = ""
    ) -> Dict[str, str]:
        """生成分组播放列表文件"""
